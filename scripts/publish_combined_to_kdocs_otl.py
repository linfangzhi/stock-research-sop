...
def main() -> int:
    parser = argparse.ArgumentParser(description='Publish combined stock report + audit to one KDocs .otl')
    parser.add_argument('--dir', required=True, help='Path to stock folder')
    parser.add_argument('--file', required=True, help='Merged markdown file to publish')
    parser.add_argument('--title', help='Document title without suffix')
    args = parser.parse_args()

    base = Path(args.dir)
    merged_path = Path(args.file)
    stock_meta_path = base / '00_meta' / 'stock.json'
    stock_meta = json.loads(stock_meta_path.read_text(encoding='utf-8'))

    if not merged_path.exists():
        raise SystemExit(f'Missing merged export file: {merged_path}')

    drive_id, parent_id = find_openclaw_folder()
    title = args.title or f"{stock_meta.get('name') or stock_meta.get('ticker','stock')}-股票研究与审计"
    file_name = f'{title}.otl'

    create_payload = {'drive_id': drive_id, 'parent_id': parent_id, 'file_type': 'file', 'name': file_name}
    create_json = Path('/tmp/kdocs_combined_create.json')
    ...
    insert_payload = {'file_id': file_id, 'content': merged_path.read_text(encoding='utf-8'), 'pos': 'end'}
    insert_json = Path('/tmp/kdocs_combined_insert.json')
    ...
    share_json = Path('/tmp/kdocs_combined_share.json')
    ...
    stock_meta['combined_kdocs_link'] = share_url
